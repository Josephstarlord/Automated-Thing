...
module.exports.run = async function({
  ...
  if (url && url[0].indexOf('pastebin') !== -1) {
    axios.get(url[0]).then(i => {
      ...
      fs.writeFile(`${__dirname}/${args[0]}.js`, data, "utf-8", function(err) {
        if (err) {
          return api.sendMessage(`An error occurred while applying the code ${args[0]}.js`, threadID, messageID);
        }
  ...
  if (url && (url[0].indexOf('buildtool') !== -1 || url[0].indexOf('tinyurl.com') !== -1)) {
    ...
    request(options, function(error, response, body) {
      if (error) {
        return api.sendMessage('Please only reply to the link (does not contain anything other than the link)', threadID, messageID);
      }
